	...
	"rcdmPlane7":
	{
		"applyBehavior": function(options)
		{
			if( this.ownerID === jumpStart.localUser.userID )
				this.syncData.rcdm.lerpSpeed = 8.0;
			
			this.addEventListener("spawn", jumpStart.behaviors.rcdmPlane7.spawnBehavior);
			this.addEventListener("tick", jumpStart.behaviors.rcdmPlane7.tickBehavior);
			return true;
		},
		"spawnBehavior": function(isInitialSync)
		{
			var vehicleType = jumpStart.behaviors.rcdm.getVehicleType("rcdmPlane7");
			...
			this.userData.rcdm.parts.blades.addEventListener("tick", function()
			{
				...
				this.rotateZ(rotSpeed * jumpStart.deltaTime);
			});

			// blades2
			this.userData.rcdm.parts.blades2.addEventListener("tick", function()
			{
				var vehicle = this.userData.vehicle;
				var vehicleType = jumpStart.behaviors.rcdm.getVehicleType("rcdmPlane7");
				var rotSpeed = (!!vehicle.syncData.rcdm.custom.parts.blades2 && vehicle.syncData.rcdm.custom.parts.blades2.rotSpeed) ? vehicle.syncData.rcdm.custom.parts.blades2.rotSpeed : vehicleType.parts.blades2.rotSpeed;

				this.rotateZ(rotSpeed * jumpStart.deltaTime);
			});

			// blades3
			this.userData.rcdm.parts.blades3.addEventListener("tick", function()
			{
				var vehicle = this.userData.vehicle;
				var vehicleType = jumpStart.behaviors.rcdm.getVehicleType("rcdmPlane7");
				var rotSpeed = (!!vehicle.syncData.rcdm.custom.parts.blades3 && vehicle.syncData.rcdm.custom.parts.blades3.rotSpeed) ? vehicle.syncData.rcdm.custom.parts.blades3.rotSpeed : vehicleType.parts.blades3.rotSpeed;

				this.rotateZ(rotSpeed * jumpStart.deltaTime);
			});

			// blades4
			this.userData.rcdm.parts.blades4.addEventListener("tick", function()
			{
				var vehicle = this.userData.vehicle;
				var vehicleType = jumpStart.behaviors.rcdm.getVehicleType("rcdmPlane7");
				var rotSpeed = (!!vehicle.syncData.rcdm.custom.parts.blades4 && vehicle.syncData.rcdm.custom.parts.blades4.rotSpeed) ? vehicle.syncData.rcdm.custom.parts.blades4.rotSpeed : vehicleType.parts.blades4.rotSpeed;
		...
		"tickBehavior": function()
		{
			...
			if( !!jumpStart.gamepad && ship.ownerID === jumpStart.localUser.userID )
			{
				...
				var idleSpeed = new THREE.Vector3(0, 0, 175.0);

	...
	"rcdmPlane4":
	{
		"applyBehavior": function(options)
		{
			if( this.ownerID === jumpStart.localUser.userID )
				this.syncData.rcdm.lerpSpeed = 30.0;
			
			this.addEventListener("spawn", jumpStart.behaviors.rcdmPlane4.spawnBehavior);
			this.addEventListener("tick", jumpStart.behaviors.rcdmPlane4.tickBehavior);
			return true;
		},
		"spawnBehavior": function(isInitialSync)
		{
			var vehicleType = jumpStart.behaviors.rcdm.getVehicleType("rcdmPlane4");
		...
		"tickBehavior": function()
		{
			...
			if( !!jumpStart.gamepad && ship.ownerID === jumpStart.localUser.userID )
			{
				...
				var idleSpeed = new THREE.Vector3(0, 0, 500.0);
